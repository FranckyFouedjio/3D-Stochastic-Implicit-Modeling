...
{
  if(sd_vector[1]<0){domain=3}
  else if (sd_vector[2]<0){domain=2}
  else if (sd_vector[3]<0){if(sd_vector[4]<0){domain=1}else{domain=6}}
  else if (sd_vector[5]<0){domain=5}else{domain=4}
  return(domain)
}
...
R_weight_computing=function(r,target_full_grid_l,target_grid,cellx,celly,cellz,v_current,v,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l)
{
  ...
  vp=rep(0,n_target_full_grid_l+1)
  for ( l in 1:(n_target_full_grid_l+1))
  {
  ...
  for ( l in 1:n_target_full_grid_l)
  {
    U1[l,1]=R_loss_function_1(sign(-2*indicator_set[[l]]+1),phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][1:length(indicator_set[[l]])])
    U1[l,2]=R_loss_function_2(phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][-1:-length(indicator_set[[l]])]) 
    U1[l,3]=R_loss_function_3(phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][-1:-length(indicator_set[[l]])]) 
  }
  
  U2=rep(NA,n_target_full_grid_l+1)
  for ( l in 1:(n_target_full_grid_l+1))
  {
...
R_global_weight_computing=function(target_full_grid_l,target_grid,cellx,celly,cellz,v_current,v,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l)
{
  ...
  RR2=matrix(NA,ncol=length(EE),nrow=n_target_full_grid_l+1)
  
  # for ( k in 1:length(EE))
  # {
  #   Result=R_weight_computing(rep(EE[k],n_target_full_grid_l),target_full_grid_l,target_grid,cellx,celly,cellz,v_current,v,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l)
  #   RR1[,,k]=Result[[1]]
  #   RR2[,k]=Result[[2]]
  # }
  Result=foreach(k=1:length(EE))%dopar%
  {
    R_weight_computing(rep(EE[k],n_target_full_grid_l),target_full_grid_l,target_grid,cellx,celly,cellz,v_current,v,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l)
  }
  ...
  WW1[which(WW1==0)]=1e-20
  WW2[which(WW2==0)]=1e-20
  WWN1=WW1[1,1]/WW1
  WWN2=WW1[1,1]/WW2
  WWN1[,3]=30*WWN1[,3]
  #WWN1[2,3]=2*WWN1[2,3]
...
R_global_PPO=function(target_full_grid_l,target_grid,cellx,celly,cellz,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l,niter)
{
  ...
  for(l in 1:n_target_full_grid_l)
  {
    v_current[[l]]=rnorm((mygrid$M.ext*mygrid$N.ext*mygrid$L.ext) +2,mean=0,sd=1) 
    v[[l]]=rnorm((mygrid$M.ext*mygrid$N.ext*mygrid$L.ext) +2,mean=0,sd=1)
  }
  ...
  weights_m=R_global_weight_computing(target_full_grid_l,target_grid,cellx,celly,cellz,v_current,v,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l)
  ...
  for ( i in 1:niter)
  {
    ...
    R_loss_function_min=function(r)
    {
      ...
      for ( l in 1:n_target_full_grid_l)
      {
        U1[l,1]=weights_m[[1]][l,1]*R_loss_function_1(sign(-2*indicator_set[[l]]+1),phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][1:length(indicator_set[[l]])])
        U1[l,2]=weights_m[[1]][l,2]*R_loss_function_2(phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][-1:-length(indicator_set[[l]])]) 
        U1[l,3]=weights_m[[1]][l,3]*R_loss_function_3(phi_sim[[l]][-1:-n_target_grid][-1:-length(cat_data)][-1:-length(indicator_set[[l]])]) 
      }
    ...
    r_res=nloptr(x0=rep(0,n_target_full_grid_l), eval_f=R_loss_function_min, lb = rep(-pi,n_target_full_grid_l), ub = rep(pi,n_target_full_grid_l),opts=list("algorithm"="NLOPT_LN_COBYLA","xtol_rel"=1.0e-20,maxeval=50))
...
R_multiple_run_global_PPO=function(target_full_grid_l,target_grid,cellx,celly,cellz,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l,niter,path,r)
{
  library(foreach)
  library(doParallel)
  
  foreach(l=((r-1)*14+1):(r*14)) %dopar%
  { 
    ...
    source("./code/functions/fftma_function.R")
    source("./code/functions/global_ppo_computing_function.R")
    Res=R_global_PPO(target_full_grid_l,target_grid,cellx,celly,cellz,phi_current_l,cat_data,indicator_set,min_sill_l,max_sill_l,min_range_l,max_range_l,vol_min_l,vol_max_l,niter)

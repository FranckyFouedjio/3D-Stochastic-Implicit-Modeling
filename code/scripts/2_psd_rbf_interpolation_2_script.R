...
##########################################################################################################################################
#                                                                                                                                        #
#                                     SYNTHETIC CASE STUDY:  PSD RBF INTERPOLATION 2 (FORMATION 2)                                       #
#                                                                                                                                        #
##########################################################################################################################################
...
rbf_result=list()
data_p_=data_p=as.data.frame(fread(paste("./outputs/data/","data_points_rbf_",lithology_names[2],".csv",sep="")))
ii_index=which(data_points_binary[,2]==1)
...
grid_p=as.data.frame(fread(paste("./inputs/data/","full_grid_points_",lithology_names[2],".csv",sep="")))
rbf_result[[1]]=R_rbf_smoothing_interpolating(data_p,grid_p,smooth_param=0,type=0)
cat("RBF Interpolation Signed Distance Function Formation 2  Completed","\n")  
...
fwrite(cbind(full_grid_points[[1]],SD=sdf[[1]]),paste("./outputs/data/rbf/","psd_rbf_sdf_",2,".csv",sep=""),row.names=FALSE)

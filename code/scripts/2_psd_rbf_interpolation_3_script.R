...
##########################################################################################################################################
#                                                                                                                                        #
#                                     SYNTHETIC CASE STUDY:  PSD RBF INTERPOLATION 3 (FORMATION 3)                                       #
#                                                                                                                                        #
##########################################################################################################################################
...
rbf_result=list()
data_p_=data_p=as.data.frame(fread(paste("./outputs/data/","data_points_rbf_",lithology_names[3],".csv",sep="")))
ii_index=which(data_points_binary[,3]==1)
...
grid_p=as.data.frame(fread(paste("./inputs/data/","full_grid_points_",lithology_names[3],".csv",sep="")))
rbf_result[[1]]=R_rbf_smoothing_interpolating(data_p,grid_p,smooth_param=0,type=0)
cat("RBF Interpolation Signed Distance Function Formation 3  Completed","\n")  
...
fwrite(cbind(full_grid_points[[1]],SD=sdf[[1]]),paste("./outputs/data/rbf/","psd_rbf_sdf_",3,".csv",sep=""),row.names=FALSE)

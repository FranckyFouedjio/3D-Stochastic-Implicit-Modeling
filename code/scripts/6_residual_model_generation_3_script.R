...
##########################################################################################################################################
#                                                                                                                                        #
#                                       SYNTHETIC CASE STUDY: RESIDUAL MODEL REALIZATIONS (FORMATION 3)                                  #
#                                                                                                                                        #
##########################################################################################################################################
...
write_feather(as.data.frame(sample_simu),"./outputs/data/unconditional_realizations/sampling_order_3.feather")
...
{
  library(RGeostats)
  vario_model=model.create(vartype=3,range= range[sample_simu[l],3],sill=sill[sample_simu[l],3],ndim=3)
  seed_set=sample(1:1234567890,1)
  simtub(dbout=db_grid,model=vario_model,neigh=neighborhood,nbsimu=1,mean=0,seed=seed_set)[,5]
}
closeAllConnections()
write_feather(rbindlist(list(residual_field)),"./outputs/data/unconditional_realizations/residual_field_3.feather")
T2=Sys.time()
difftime(T2,T1)

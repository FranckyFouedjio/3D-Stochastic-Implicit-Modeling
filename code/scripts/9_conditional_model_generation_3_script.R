...
###############################################################################################
#                                                                                             #
#           SYNTHETIC CASE STUDY: CONDITIONAL MODEL GENERATION (FORMATION 3)                  #
#                                                                                             #
###############################################################################################
...
if(!file.exists("./outputs/figures/9_conditional_model_generation/3")){dir.create("./outputs/figures/9_conditional_model_generation/3")}
if(!file.exists("./outputs/figures/9_conditional_model_generation/3/scores_rqp")){dir.create("./outputs/figures/9_conditional_model_generation/3/scores_rqp")}
...
contact_points_cat[[1]]=readRDS('./inputs/data/contact_points_cat.RDS')[[3]]

data_points_binary=as.data.frame(fread("./outputs/data/drillhole_data_binary.csv",header=TRUE))
psd_data=as.data.frame(fread("./outputs/data/psd_data.csv")[,3])
lithology_names=1:4
...
pca_prior_sdf=readRDS(paste("./outputs/data/pca_unconditional_realizations/pca_unconditional_sdf_",3,".rds",sep=""))
...
{
  png(paste("./outputs/figures/9_conditional_model_generation/3/scores_rqp/Con_Scores_",l,"_",l+1,".png",sep=""),width=5,height=5,units="in",res=300)
...
saveRDS(LUR_Res,"./outputs/data/conditional_model_generation/RQP_3.RDS")
...
write_feather(as.data.frame(signed_distance_fields_posterior_),paste("./outputs/data/conditional_model_generation/conditional_sdf_model_",3,".feather",sep=""))
T2=Sys.time() 
difftime(T2,T1)
